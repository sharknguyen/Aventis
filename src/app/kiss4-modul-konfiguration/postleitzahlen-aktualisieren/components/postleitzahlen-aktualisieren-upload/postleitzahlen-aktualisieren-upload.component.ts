...
import { getRequestHeaderFromLocalStorage, getUploadUrl } from '@shared/utilites';
import { DxFileUploaderComponent } from 'devextreme-angular';

@Component({
    selector: 'kiss-postleitzahlen-aktualisieren-upload',
    templateUrl: './postleitzahlen-aktualisieren-upload.component.html',
    styleUrls: ['./postleitzahlen-aktualisieren-upload.component.scss']
})
@SetClassRight('CtlPostleitzahlenAktualisieren')
export class PostleitzahlenAktualisierenUploadComponent {
    ...
    uploadUrl = getUploadUrl('api/Basis/UploadBaPlz');
    requestHeader = getRequestHeaderFromLocalStorage();
    ...
    onValueChanged(event) {
        if (event.value[0] && event.value[0].size > this.maxFileSize) {
            this.remainingMessage.showMessage(this.translateService.instant('PostleitzahlenAktualisieren.Message.MaxFileSizeMessage'));
        }
    }
    // #endregion
